...
${DOCKER_CLI} buildx build -f ../docker-compose/Dockerfile_ui --platform linux/${TARGET_ARCH} -t ${IMAGE_NAME}:${IMAGE_TAG} --output "type=docker,name=${IMAGE_NAME}:${IMAGE_TAG}" --build-context project=../ctrlx-datalayer-ui .
